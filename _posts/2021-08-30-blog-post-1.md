...
\end{align*}
$$
$P_0$持有$$([ \lambda_{\mathsf{f}} ]_1, [ \lambda_{\mathsf{f}} ]_2)$$，$P_j, j \in \{1,2\}$持有$$([ \lambda_{\mathsf{f}} ]_j, \mathsf{f} + \lambda_{\mathsf{f}})$$，服务器映射$[\chi]$和$$ \gamma_{\mathsf{x}}\gamma_{\mathsf{y}} + \psi $$到$[\gamma_{\mathsf{f}}]$和$ \mathsf{f} + \gamma_{\mathsf{f}} $。$P_j, j \in \{1,2\}$ 利用随机数 $$ \mathsf{r} \in \mathbb{Z}_{2^l} $$生成$[\psi] $，设置$$ [ \psi ]_1 = \mathsf{r} $$和$$ [ \psi ]_2 = \psi - \mathsf{r} $$ 。$P_j, j \in \{1,2\}$持有$$ [ \Gamma_{\mathsf{xy}} ] = [ \alpha_{\mathsf{x}}\alpha_{\mathsf{y}} ] $$，重构：
