...
算法中参数的计算：
$$ r = a + x - \eta \cdot L \; \tag{1}$$  

$$ r = r_1 + r_2 + r_3 - \delta_e \cdot L \; \tag{2}$$

$$ r_i = a_i + x_i - \beta_i \cdot L \quad \forall i \in \{1,2,3\} \tag{3}$$

$$ x = x_1 + x_2 + x_3 - \alpha_e \cdot L \tag{4}$$
...
$$ a = a_1 + a_2 + a_3 - \theta_e \cdot L \tag{5}$$
...
$$ \mathsf{DReLU}(a) = \mathsf{MSB}(a_1) \oplus \mathsf{MSB}(a_2) \oplus \mathsf{MSB}(a_3) \oplus \mathsf{wrap}_3(2a_1, 2a_2, 2a_3, L) \oplus 1 \tag{5}$$
...
- 通过等式5计算共享$$ [\![ b]\!]^2, b=\mathsf{DReLU}(a) $$
